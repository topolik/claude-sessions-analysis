...
import sqlite3
import pandas as pd
from db_utils import table_and_cols_exist
...
def profile_database():
    ...
    for table in tables:
        ...
        desc = {
            ...
            "message_parts": "Unified relational storage for auxiliary content part types (thinking, signature, etc.)."
        }.get(table, "Dynamic user table.")
    ...
    for table in discovered_tables:
        ...
        tables_cols[table] = [row[1] for row in cursor.fetchall()]
    ...
    # Event Types
    if table_and_cols_exist(conn, "events", ["event_type"]):
    ...
    if table_and_cols_exist(conn, "events", ["event_type", "role"]):
    ...
    if table_and_cols_exist(conn, "messages", ["content"]):
    ...
    if table_and_cols_exist(conn, "tool_calls", ["tool_name", "tool_use_id"]) and table_and_cols_exist(conn, "tool_results", ["tool_use_id", "output_content"]):
    ...
    if table_and_cols_exist(conn, "events", ["input_tokens", "output_tokens", "cache_read_tokens", "cache_creation_tokens"]):
